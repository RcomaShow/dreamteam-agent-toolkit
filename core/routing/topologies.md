...
A delegated Lean candidate includes an explicit non-zero **root Sonnet executive forecast** in addition to the bounded worker and any independent verifier. `topology=lean` is a model contract: configuring a non-Sonnet executive is invalid rather than silently creating an undeclared hybrid topology.
...
Opus owns cross-domain C3 decisions and final acceptance. Sonnet acts as bounded lead and independent reviewer. Haiku handles bounded volume. Frontier requires explicit Opus, Sonnet, and Haiku accounting and remains a quality route until paired benchmarks prove economic and token value for a task bucket.

## Routes

- `BLOCKED`: the request cannot proceed without violating a forecast budget, strict runtime capability, topology contract, or required ownership invariant.
